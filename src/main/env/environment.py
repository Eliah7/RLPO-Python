    ...
    def get_observation(self, action=np.inf):
        if self.action_type == "discrete":
            ...
            print("ACTION REWARD {}".format(
                    calculate_reward(action, self.load_data[:, 1], self.load_data[:, 4])))
            ...
            if calculate_reward(np.array(action), self.load_data[:, 1], self.load_data[:, 4]) > self.current_reward:  # do not allow a state with less priority
                action_load_data = self.load_data
                action_load_data[:, 3] = np.array(action)

                power_values_from_dlf, _ = dlf_analyse(self.line_data, action_load_data, grid_name=self.grid_name)
